...
// Backup/Default Data for first run with high-quality real images
const defaultTrips = [
...
function getTrips() {
    ...
    // Force clear and reload if we want fresh images on first load
    if (!data) {
...
function renderMainTrips() {
    ...
    if (trips.length === 0) {
        packageGrid.innerHTML = '<p style="text-align: center; grid-column: 1/-1; color: var(--text-gray);">لا توجد رحلات مضافة حالياً.</p>';
        return;
    }

    packageGrid.innerHTML = trips.map(trip => `
